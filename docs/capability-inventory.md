...
| `frontend-artifacts` | Frontend app and artifact workflows | Governed base plus frontend testing/design skills and frontend UI implementation agent | Curated art-heavy builders | Repository healthcheck plus browser/app tests |
...
| `context-budget-audit` | `core` | Local template, ECC concepts adapted | Audit context/token footprint across instructions, skills, agents, profiles, and docs; recommend lazy-loading or pruning without mutating files. | Low | Cross-platform | A workspace wants evidence for token economy, large capability surfaces, or candidate pruning. | The task needs actual pruning, compaction, or runtime-global edits. |
...
| `plan-deep-skills` | `core` | Local template | Capability-audit planning workflow for skill discovery, external skill vetting, conversion risk, and skill-to-task fit before implementation handoff. | Low | Cross-platform | A plan needs capability selection, skill evaluation, skill migration review, or delegation planning. | No skill, capability, or subagent decision is involved. |
...
| `verification-loop` | `core` | Local template, ECC concepts adapted | Orchestrate validation commands by repository type without replacing healthchecks or making unapproved changes. | Low | Cross-platform | A project needs a repeatable readiness loop or validation contract before handoff. | The user asked for a single targeted command or validation would require unavailable external services. |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `api_backend_engineer` | `optional` | API design, backend service implementation, routing, contracts, validation, and integration boundaries. | Strong Codex-oriented model for backend implementation. | `workspace-write` | Medium | API routes, service modules, request/response contracts, backend validation, or integration boundaries need a bounded owner. | The task is frontend UI, data pipeline/dbt, dependency setup, security review, broad architecture, or simple enough for the main agent. |
...
| `data_catalog_taxonomist` | `optional` | Data catalog structure, entity hierarchy, glossary, metadata standards, lineage organization, stewardship, and discovery-ready documentation. | Smaller or mid-size analytical model is acceptable for bounded metadata work; escalate for ambiguous enterprise taxonomy. | `workspace-write` | Medium | Catalog, hierarchy, glossary, lineage, ownership, or metadata organization needs a bounded owner. | The task is transformation implementation, modeling, or dashboard design. |
| `data_discovery_researcher` | `optional` | Read-only data source discovery, upstream research, schema reconnaissance, ownership, constraints, and evidence-backed handoff questions. | Smaller or mid-size analytical model is acceptable for bounded read-only research; escalate for complex source ambiguity. | `read-only` | Low | Source systems, grain, ownership, freshness, access, or lineage are unclear before implementation. | The source contract is already known and implementation can start. |
...
| `docs_researcher` | `optional` | Read-only primary documentation, release notes, APIs, framework behavior, and source-backed implementation facts. | Smaller or mid-size research model is acceptable for bounded docs review; escalate when current docs determine high-risk implementation. | `read-only` | Low | Implementation or planning depends on current official docs, API behavior, release notes, or framework changes. | The task is data-source discovery, security analysis, or already covered by validated local project docs. |
| `frontend_ui_engineer` | `optional` | Frontend web interfaces, HTML artifacts, responsive UI implementation, browser validation, and user-facing interaction quality. | Strong Codex-oriented model for frontend implementation. | `workspace-write` | Medium | Frontend web, HTML/CSS/JavaScript, responsive layout, accessibility, or browser-facing interaction work needs a bounded owner. | The task is BI/dashboard-specific, backend/API, dependency setup, security review, broad product design, or simple enough for the main agent. |
...
| `test_automation_engineer` | `optional` | Automated tests, fixtures, smoke checks, validation commands, and regression coverage for scoped changes. | Strong Codex-oriented model for test implementation; smaller model acceptable for narrow test wiring. | `workspace-write` | Medium | Tests, fixtures, smoke checks, validation scripts, or regression coverage need a bounded owner. | Final QA review is needed, a domain specialist should own tightly coupled test work, or the change is too small to delegate. |
...
| `workspace_implementer` | `core` | General practical implementation fallback for scoped repository edits when no specialist subagent is a better fit. | Strong Codex-oriented model for implementation. | `workspace-write` | Medium | A planned practical change needs scripts, configs, manifests, docs tied to the change, patches, or small/medium refactors and no specialist applies. | The task is simple enough for the main agent, ambiguous, architectural, destructive, final approval, push, or covered by a specialist such as security, QA, docs, packages, Git, data, BI, frontend UI, API/backend, AGENTS.md, or skills. |
