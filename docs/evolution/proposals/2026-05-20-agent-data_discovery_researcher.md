# Capability Proposal: data_discovery_researcher

Date: 2026-05-20
Kind: `agent`
Status: `optional`

## Purpose

Read-only specialist for data source discovery, upstream research, schema reconnaissance, ownership, constraints, and evidence-backed handoff questions.

## Existing Capability Check

- Existing agents reviewed: `data_pipeline_engineer`, `data_science_modeler`, `dashboard_visualization_specialist`.
- Existing skills reviewed: dbt, Power BI, and continuous-evolution skills.
- Why reuse is insufficient: existing data agents start at implementation, modeling, or visualization. None owns read-only source discovery, owner research, source constraints, or pre-implementation handoff questions.

## Automation Decision

- `proposal`: safe to review without runtime effects.
- `apply`: allowed because the role is optional, evidence-backed, and non-duplicative.

## Required Follow-Up

- Include the agent in the `data-bi` profile.
- Validate with `python3 skills/migrate-to-codex/scripts/cli.py --validate-target .`.
- Validate with `python3 scripts/evolve-workspace.py --strict`, `python3 scripts/validate-skills.py --strict`, and `bash scripts/healthcheck.sh --strict`.
